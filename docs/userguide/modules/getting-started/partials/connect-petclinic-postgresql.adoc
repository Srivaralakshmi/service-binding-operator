...
[source,yaml]
----
include::attachment$petclinic-postgresql-binding.yaml[tag=service-binding]
----
+++ </div></details> +++
...
----
kubectl apply -f link:{attachmentsdir}/petclinic-postgresql-binding.yaml[pass:a[{site-url}/{page-component-name}/{page-module}/{attachmentsdir}/petclinic-postgresql-binding.yaml]] -n {quickstart-namespace}
----
...
`/bindings/spring-petclinic-postgresql` directory you\'ll see all the values
